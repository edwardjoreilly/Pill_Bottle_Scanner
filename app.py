...
try:
	with connection:
		with connection.cursor() as cursor:
			cursor.execute("CREATE TABLE users (id SERIAL PRIMARY KEY, firstname VARCHAR (100) NOT NULL, lastname VARCHAR (100) NOT NULL, username VARCHAR (50) NOT NULL, password VARCHAR (255) NOT NULL, email VARCHAR (50) NOT NULL, securityq1 VARCHAR (50) NOT NULL, securityq2 VARCHAR (50) NOT NULL, faceimagename VARCHAR (100));")
...
try:
	with connection:
		with connection.cursor() as cursor:
			cursor.execute("CREATE TABLE appointments (id SERIAL PRIMARY KEY, username VARCHAR (50) NOT NULL, appointmentname VARCHAR (50) NOT NULL, appdate DATE NOT NULL, starttime TIME NOT NULL, endtime TIME NOT NULL, remindertime TIME NOT NULL, doctorname VARCHAR (50) NOT NULL, location VARCHAR (50) NOT NULL);")
except psycopg2.errors.DuplicateTable:
	pass
...
try:
	with connection:
		with connection.cursor() as cursor:
			cursor.execute("CREATE TABLE reminders (id SERIAL PRIMARY KEY, username VARCHAR (50) NOT NULL, medicinename VARCHAR (50) NOT NULL, remindertime TIME NOT NULL, monday BOOLEAN, tuesday BOOLEAN, wednesday BOOLEAN, thursday BOOLEAN, friday BOOLEAN, saturday BOOLEAN, sunday BOOLEAN);")
except psycopg2.errors.DuplicateTable:
	pass
...
def addApp():
	...
	if request.method == 'POST' and 'starttime' in request.form and 'remindertime' in request.form:
		starttime = request.form['starttime']
		newStartTime = starttime.replace(':', '')
		starttime = int(newStartTime[:4])

		remindertime = request.form['remindertime']
		newReminderTime = remindertime.replace(':', '')
		remindertime = int(newReminderTime[:4])

		endtime = request.form['endtime']
		newEndTime = endtime.replace(':', '')
		endtime = int(newEndTime[:4])

		# this needs to be a date
		dateRequested = request.form['date']
		dateRequested = dateRequested.replace('-', '/')
		dateRequested = datetime.datetime.strptime(dateRequested, "%Y/%m/%d").date()
		print('dateRequested: ', dateRequested)

 
	# Check if all fields POST requests exist (user submitted form)
	if request.method == 'POST' and 'starttime' in request.form and 'endtime' in request.form and 'date' in request.form and 'appname' in request.form and 'remindertime' in request.form and (remindertime <= starttime) and (starttime <= endtime) and dateRequested >= presentdate:
		...
		cursor.execute('SELECT * FROM appointments WHERE username = %s AND appointmentname = %s AND starttime = %s AND endtime = %s', (session['username'], appname, starttime, endtime))
		account = cursor.fetchone()
		# If appointment exists show error and validation checks
		if account:
			flash('Appointment already exists!')
		else:
			# Account doesnt exist and the form data is valid, now insert new account into users table
			cursor.execute("INSERT INTO appointments (username, starttime, endtime, remindertime, appdate, appointmentname, location, doctorname) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)", (session['username'], starttime, endtime, remindertime, dateRequested, appname, location, doctorname))
			connection.commit()
			flash('Your appointment has been added!')
	elif request.method == 'POST':
		# Form is empty... (no POST data)
		flash('Please fill out the form with a date, start, and end time that hasn\'t passed and enter a reminder time that occurs before the start time!')
...
def addReminder():
	...
	# Check if all fields POST requests exist (user submitted form)
	if request.method == 'POST' and 'remindertime' in request.form and 'medication' in request.form and ('Monday' in request.form or 'Tuesday' in request.form or 'Wednesday' in request.form or 'Thursday' in request.form or 'Friday' in request.form or 'Saturday' in request.form or 'Sunday' in request.form):
		# Create variables for easy access
		medicinename = request.form['medication']
		remindertime = request.form['remindertime']
		...
		# Check if account exists using MySQL
		cursor.execute('SELECT * FROM reminders WHERE username = %s AND remindertime = %s AND medicinename = %s', (session['username'], remindertime, medicinename))
		reminder = cursor.fetchone()
		# If appointment exists show error and validation checks
		if reminder:
			flash('Reminder already exists!')
		else:
			# Account doesnt exist and the form data is valid, now insert new account into users table
			cursor.execute("INSERT INTO reminders (username, remindertime, medicinename, monday, tuesday, wednesday, thursday, friday, saturday, sunday) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (session['username'], remindertime, medicinename, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday))
			connection.commit()
			flash('Your reminder has been added!')
	elif request.method == 'POST':
		# Form is empty... (no POST data)
		flash('Please fill out the form and please select at least one day of the week!')
...
def deleteApp():
	...
	# Delete the post
	try:
		cursor.execute("SELECT * FROM appointments WHERE appointmentname = %s AND username = %s", (appName, session['username']))
		appointment = cursor.fetchone()
		#imagename = post['imagename']
	
		#if(os.path.exists(os.path.join(app.config['UPLOAD_FOLDER'], imagename))):
			#os.remove(os.path.join(app.config['UPLOAD_FOLDER'], imagename))
		cursor.execute("DELETE FROM appointments WHERE appointmentname = %s AND username = %s", (appName, session['username']))
		connection.commit()
	except:
		flash("Please enter only only one appointment name at a time.")
		connection.rollback()
...
def deleteReminder():
	...
	# Delete the post
	try:
		cursor.execute("SELECT * FROM reminders WHERE medicinename = %s AND username = %s", (medicineName, session['username']))
		reminder = cursor.fetchone()
		#imagename = post['imagename']
	
		#if(os.path.exists(os.path.join(app.config['UPLOAD_FOLDER'], imagename))):
			#os.remove(os.path.join(app.config['UPLOAD_FOLDER'], imagename))
		cursor.execute("DELETE FROM reminders WHERE medicinename = %s AND username = %s", (medicineName, session['username']))
		connection.commit()
	except:
		flash("Please enter only only one medicine name at a time.")
		connection.rollback()
...
def register():
	...
	if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'password2' in request.form and 'email' in request.form  and 'firstname' in request.form  and 'lastname' in request.form  and 'securityq1' in request.form  and 'securityq2' in request.form:
		...
		if account:
			...
			# Account doesnt exist and the form data is valid, now insert new account into users table
			cursor.execute("INSERT INTO users (firstname, lastname, username, password, email, securityq1, securityq2, faceimagename) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)", (firstname, lastname, username, _hashed_password, email, securityq1, securityq2, filename))
			connection.commit()
			sent = True
...
def facelogin():
	...
	if request.method == 'POST' and 'username' in request.form:
		...
		if account:
			...
			photoName = account['faceimagename']
			#grab photo from the saved uploads folder
			photo = 'static/uploads/' + photoName

			try:
				#Taking picture
				cap = cv2.VideoCapture(0)
				ret, frame = cap.read()
				cv2.imwrite('static/uploads/Picture.jpg', frame)
			...
			#Analyzing User
			baseimg = face_recognition.load_image_file(photo) #image stored in database
			...
			sampleimg = face_recognition.load_image_file("static/uploads/Picture.jpg") #photo that was taken
			sampleimg = cv2.cvtColor(sampleimg, cv2.COLOR_BGR2RGB)
		
			#Attempt to compare faces
			try:
				...
				#Unable to find face in taken photo
				flash("Unable to find face")
			...
			if resultstring == "[True]":
				...
				#login failed, retry
				flash("Login Failed. Please login with password.")
		else:
...
def appointments():
	...
	if 'loggedin' in session:
		...
		# Display user's previous entries
		cursor.execute('SELECT * FROM appointments WHERE username = %s', (session['username'],))
		appointments = cursor.fetchall()

		return render_template('appointments.html', sent=sent, appointments=appointments)
...
def reminders():
	...
	if 'loggedin' in session:
		...
		# Display user's previous entries
		cursor.execute('SELECT * FROM reminders WHERE username = %s', (session['username'],))
		reminders = cursor.fetchall()

		return render_template('reminders.html', sent=sent, reminders=reminders)
